...
private:
  [[nodiscard]] auto distance(PosVec::const_iterator star1, PosVec::const_iterator star2,
                              UInt expansion_coefficient) const -> UInt
  {
    auto [x1, y1] = *star1;
    auto [x2, y2] = *star2;
    ...
    return (x2 - x1) + (y2 - y1) + expansion_coefficient * (
             horiz_empty(x1, x2) + vert_empty(y1, y2));
  }

  [[nodiscard]] auto horiz_empty(UInt x1, UInt x2) const -> UInt
  {
    return std::count_if(horiz_stars_.begin() + static_cast<std::int64_t>(x1),
                         horiz_stars_.begin() + static_cast<std::int64_t>(x2), [](UInt count) {
                           return count == 0;
                         });
  }

  [[nodiscard]] auto vert_empty(UInt x1, UInt x2) const -> UInt
  {
    return std::count_if(vert_stars_.begin() + static_cast<std::int64_t>(x1),
                         vert_stars_.begin() + static_cast<std::int64_t>(x2), [](UInt count) {
                           return count == 0;
                         });
  }
...
auto main() -> int try {
  ...
  std::cout << "Star map distance (coeff 1): " << map.star_distances(1) << '\n';
