...
enum class Opcode { cpy, inc, dec, jnz, tgl, out };
...
auto main() -> int
{
  ...
  // So it looks as if the code we're given takes the value in register A adds 2548 to it and then
  // outputs the binary representation starting at the least significant bit.  So we want
  // (a + 2548) to have the binary representation 1010101010 - which turns out to be 182.
  Int ra = 2730 - 2548;
  state.reg(Register::a, ra);
  state.execute();
}
